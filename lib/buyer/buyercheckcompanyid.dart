...
import 'package:milkdistributionflutter/buyer/buyerlogin.dart';
import 'package:milkdistributionflutter/milkman/milkmanlogin.dart';
import 'package:http/http.dart' as http;

class BuyerCheckCompanyId extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return BuyerCheckCompanyIdState();
  }
}

class BuyerCheckCompanyIdState extends State<BuyerCheckCompanyId> {
  ...
  Future GetCompanyIdData() async {
    ...
    if (data == 1) {
      if (responseid == 1) {
        _myServices.myBuyerCompanyId = _companyid.text;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text("CompanyId Matched"),
          duration: Duration(seconds: 3),
        ));
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => Buyerlogin(_companyid.text)));
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
