...
import 'buyercalender.dart';

class BuyerChangeMilkQuantity extends StatefulWidget {
  String sday, smonth, syear, sdate;
  int custid;
  BuyerChangeMilkQuantity(
      this.sday, this.smonth, this.syear, this.sdate, this.custid);

  @override
  State<StatefulWidget> createState() {
    return BuyerChangeMilkQuantityState(sday, smonth, syear, sdate, custid);
  }
}

class BuyerChangeMilkQuantityState extends State<BuyerChangeMilkQuantity> {
  String sday,
      smonth,
      syear,
      sdate,
  ...
  void navigateToAddProduct() async {
    Navigator.of(context)
        .push(new MaterialPageRoute(
            builder: (context) =>
                BuyerChangeMilkQuantity(sday, smonth, syear, sdate, custid)))
        .whenComplete(() {
      setState(() {
  ...
  Future<List<CustomerModel>> getCustomerDataIdWise() async {
    ...
    var response = await http.post(
        Uri.parse(_myServices.myUrl + "Customer/GetIdwiseCustomer"),
        body: JsonBody,
        headers: {"Accept": "application/json"});
  ...
  Future AddDeliveryStatusData() async {
    ...
    var response = await http.post(
        Uri.parse(_myServices.myUrl + "DeliveredStatus/AddDeliveryStatus"),
        body: JsonBody,
        headers: {"Accept": "application/json"});
    var data = jsonDecode(response.body)['Status'];
    var message = jsonDecode(response.body)['Message'];
    if (data == 1) {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (BuildContext context) => BuyerCalenderPage(custid)));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(message),
        duration: Duration(seconds: 3),
  ...
  Future AddDeliveryStatusMinMaxData() async {
    ...
    if (data == 1) {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => BuyerCalenderPage(custid)));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(message),
        duration: Duration(seconds: 3),
      ...
      child: Row(
        children: <Widget>[
          Padding(
            ...
            child: Text(
              "Milk Status",
              style: GoogleFonts.adamina(fontSize: 14.0, color: Colors.black87),
            ),
          ),
          Expanded(
            child: Container(
