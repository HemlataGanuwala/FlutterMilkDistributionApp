...
class Buyerlogin extends StatefulWidget {
  String company_id = "";
  Buyerlogin(this.company_id);
  @override
  State<StatefulWidget> createState() {
    return BuyerLoginState(company_id);
  }
}

class BuyerLoginState extends State<Buyerlogin> {
  MyServices _myServices = MyServices();
  String company_id = "";
  BuyerLoginState(this.company_id);
String getValue = "";
bool pin = true;
BuyerReg _reg;
...
@override
  ...
  Future CheckPinData() async {
    ...
    var response = await http.post(
        Uri.parse(_myServices.myUrl + "Login/GetLoginData"),
        body: JsonBody,
    ...
    if (data == 1) {
      if (responseid == 1) {
        _reg = new BuyerReg(company_id: company_id,pin: getValue, Status: 1);
      var result = await DatabaseHelper.db.deleteBuyer();
      await DatabaseHelper.db.createBuyer(_reg);    
      _myServices.myBuyerPin = getValue;    
        Navigator.push(
            context,
